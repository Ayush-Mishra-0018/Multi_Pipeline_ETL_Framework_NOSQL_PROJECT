...
public final class LogParser {
    ...
    public static ParsedLog parse(String line, int batchId) {
        ...
        try {
            ...
            parseDateAndHour(log, rawTimestamp);
            parseRequest(log, request);

            log.setStatus(Integer.parseInt(status));
            log.setBytes(parseBytes(bytes));
    ...
    private static ParsedLog malformed(ParsedLog log) {
        log.setMalformed(true);
        return log;
    }

    private static void parseDateAndHour(ParsedLog log, String rawTimestamp) {
        ...
        LocalDate date =
                LocalDate.parse(rawDate, INPUT_DATE_FORMAT);

        log.setDate(date.format(OUTPUT_DATE_FORMAT));
        log.setHour(hour);
    }

    private static void parseRequest(ParsedLog log, String request) {
    ...
    private static long parseBytes(String bytes) {
